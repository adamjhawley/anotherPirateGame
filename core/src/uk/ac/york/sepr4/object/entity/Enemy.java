...
public class Enemy extends LivingEntity {
    ...
    public void act(float deltaTime) {
        ...
        if (distanceFrom(player) <= this.range) {
    ...
    private float perfectShotAngle(Player player) {
        ...
        double time = distanceFrom(player) / (getSpeed() + getSelectedProjectileType().getBaseSpeed());
...
//        Gdx.app.log("plus", BtimePlus+"");
    ...
    private float f(Player player) {
        ...
        double fx = (1 / (Math.sqrt(2 * Math.PI) * sigma)) * Math.pow(Math.E, -(Math.pow((distanceFrom(player) - (double) this.mean), 2) / (2 * Math.pow(sigma, 2))));
